...
def save_report(user_id, username, description, photo_file_id=None):
    """Сохранение сообщения о поломке"""
    data = load_data()
...
def get_all_reports():
    """Получение всех сообщений о поломках"""
    data = load_data()
...
inter@fa.ru"""
        ...
        await query.message.reply_photo(
            photo=open(first_path, 'rb'),
            caption="🗺️ Схемы корпусов:",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад", callback_data="back_to_main")]])
        )
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    bad_words = ['мат', 'хуй', 'пизда', 'бля', 'сука', 'залупа', 'мудак', 'редиска']
    ...
    if state == 'waiting_question':
        ...
        if user_id != ADMIN_ID:
            try:
                await context.bot.send_message(
                    ADMIN_ID,
                    f"📩 Новый вопрос #{question_id} от пользователя!\n\n"
                    f"Вопрос: {text[:100]}...\n"
                    f"От: @{username}"
                )
            except Exception as e:
        ...
        if user_id != ADMIN_ID:
            try:
                admin_text = f"🔧 Новое сообщение о поломке #{report_id}!\n\n"
                admin_text += f"Описание: {text}\n"
                admin_text += f"От: @{username}\n"
                if photo_id:
                    admin_text += f"Есть фото: ID {photo_id}"
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'waiting_report':
        ...
        await update.message.reply_text("Сейчас бот не ожидает фото. Используйте кнопку 'Сообщить о поломке'.")
...
if __name__ == '__main__':
    ...
    # Запускаем веб-сервер Flask (нужен для Render)
